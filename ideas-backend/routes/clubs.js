...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    // 🛡️ JAVÍTVA: Szabályos JavaScript komment! Ellenőrizzük a feketelistát az adatbázisban
...
module.exports = function(app, pool, drive, upload, cleanupTempFile) {
  ...
  app.post('/api/attendance/:meetingId', requireAuth, async (req, res) => {
    ...
    } catch (e) { await conn.rollback(); res.status(500).json({ error: e.message }); } finaly { conn.release(); }
  });
