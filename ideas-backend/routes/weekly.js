...
// 🎯 VISSZAÁLLÍTVA: Eredeti CommonJS module.exports, hogy az index.js require-je működjön!
module.exports = function(app, pool, drive, upload, cleanupTempFile) {
  ...
  // 🎯 Atombiztos, ICU-verzióktól független Magyar Idő Generátor
  ...
  // 📊 Teljesen hurokmentesített, magyar időzónára szinkronizált profil statisztika
  ...
  // 👑 A 12 szintes Nomád-Magyar progressziós motor
  ...
  // ⚡ Memóriaalapú, azonnali szavazati erő leképzés
  ...
  // ====================================================================
  // ⚔️ KÖTEGELT (BATCH) SZAVAZÁST TÁMOGATÓ CSATATÉR VÉGPONT
  // ====================================================================
  app.get('/api/weekly/current', async (req, res) => {
    ...
    try {
      ...
      // 🎯 JAVÍTVA: Az új EXIF oszlopok lekérése beépítve a leaderboardba!
  ...
  app.get('/api/weekly/my-album', async (req, res) => {
    ...
    try {
      ...
      for (const photo of photos) {
        ...
        albumWithStats.push({
          ...photo, totalLikes, totalViews, firstPlaces, podiums, isCurrentlyActive, history
        });
      }
  ...
  app.post('/api/weekly/my-album/upload', upload.single('photo'), async (req, res) => {
    ...
    try {
      ...
      await pool.query("INSERT INTO user_photos (user_email, file_url, file_hash) VALUES (?, ?, ?)", [userEmail, result.secure_url, fileHash]);
  ...
  app.post('/api/weekly/swap-existing', async (req, res) => {
    ...
    try {
      ...
      const [existing] = await conn.query('SELECT id, swapped FROM weekly_entries WHERE topic_id = ? AND user_email = ? AND is_active = 1', [topicId, userEmail]);
      if (existing.length === 0) { await conn.rollback(); return res.status(400).json({ error: 'Még nincs aktív nevezésed!' }); }

      await conn.query('UPDATE weekly_entries SET is_active = 0 WHERE id = ?', [existing[0].id]);
      const nextSwapCount = existing[0].swapped + 1;
      await conn.query('INSERT INTO weekly_entries (topic_id, user_email, user_name, file_url, drive_file_id, swapped, ip_address, is_active, likes_count, views_count) VALUES (?, ?, ?, ?, \'\', ?, ?, 1, 0, 0)', [topicId, userEmail, userName, fileUrl, nextSwapCount, ipAddress]);
      await conn.query('UPDATE photo_users SET swap_balance = swap_balance - 1 WHERE email = ?', [userEmail]);
      await conn.commit();
      res.json({ success: true });
  ...
  app.post('/api/weekly/upload-existing', async (req, res) => {
    ...
    try {
      ...
      await pool.query('INSERT INTO weekly_entries (topic_id, user_email, user_name, file_url, drive_file_id, swapped, ip_address, is_active) VALUES (?, ?, ?, ?, \'\', 0, ?, 1)', [topicId, userEmail, userName, fileUrl, ipAddress]);
      res.json({ success: true });
  ...
  app.post('/api/weekly/swap-back', async (req, res) => {
    ...
    try {
      ...
      if (result.affectedRows === 0) { await conn.rollback(); return res.status(404).json({ error: 'A kép nem található!' }); }
  ...
  app.get('/api/weekly/next-vote', async (req, res) => {
    ...
    try {
      const [entries] = await pool.query(`
        ...
        FROM weekly_entries e
        WHERE e.topic_id = ? AND e.user_email != ? AND e.is_active = 1 
          AND e.id NOT IN (SELECT entry_id FROM weekly_votes WHERE voter_email = ?)
        ORDER BY e.views_count ASC, RAND() LIMIT 1
  ...
  app.post('/api/weekly/vote', async (req, res) => {
    ...
    try {
      await conn.beginTransaction();
      const [existing] = await conn.query('SELECT id FROM weekly_votes WHERE entry_id = ? && voter_email = ?', [entryId, userEmail]);
      ...
      const isRealMasterOfThisRoom = userEmail && assignedMasterEmail && userEmail.toLowerCase().trim() === assignedMasterEmail.toLowerCase().trim();
      ...
      if (voteType === 'pass') calculatedPoints = 0;
        ...
        const [masterVotesCount] = await conn.query("SELECT COUNT(*) as count FROM weekly_votes v JOIN weekly_entries e ON v.entry_id = e.id WHERE e.topic_id = ? AND v.voter_email = ? AND v.vote_type = 'master'", [topicId, userEmail]);
        if ((masterVotesCount[0]?.count || 0) >= 5) { await conn.rollback(); return res.status(400).json({ error: 'Már elhasználtad az 5 bírói voksod!' }); }
        calculatedPoints = 10; 
      } else {
  ...
  app.post('/api/weekly/claim-referral', async (req, res) => {
    ...
    try {
      ...
      if (referrerRows.length === 0) return res.status(400).json({ error: 'Nincs ilyen kód!' });

      const referrerEmail = referrerRows[0].email;
      if (referrerEmail === userEmail) return res.status(400).json({ error: 'Saját magadét nem adhatod meg!' });
  ...
  app.get('/api/weekly/past', async (req, res) => {
    try { 
      const [rows] = await pool.query("SELECT t.*, u.name AS master_name FROM weekly_topics t LEFT JOIN photo_users u ON t.master_email = u.email WHERE t.end_date < ? AND t.status = 'approved' ORDER BY t.end_date DESC", [getLocalMySQLNow()]); 
      res.json(rows); 
    } catch (err) { res.status(500).json({ error: 'Hiba' }); }
  });
  ...
  app.get('/api/weekly/my-stats', async (req, res) => {
    const { userEmail } = req.query;
    try {
      const [pastTopics] = await pool.query("SELECT * FROM weekly_topics WHERE end_date < ? AND status = 'approved' ORDER BY end_date DESC", [getLocalMySQLNow()]);
      let podiums = { first: 0, second: 0, third: 0 }, history = [];

      for (const topic of pastTopics) {
        const [entries] = await pool.query("SELECT id, user_email, user_name, file_url, drive_file_id, likes_count, views_count FROM weekly_entries WHERE topic_id = ? AND is_active = 1 ORDER BY likes_count DESC, views_count ASC", [topic.id]);
        const userIndex = entries.findIndex(e => e.user_email === userEmail);
        if (userIndex !== -1) {
          const rank = userIndex + 1, entry = entries[userIndex];
          if (rank === 1) podiums.first++; else if (rank === 2) podiums.second++; else if (rank === 3) podiums.third++;
          history.push({ topic_title: topic.title, topic_title_en: topic.title_en, start_date: topic.start_date, end_date: topic.end_date, rank, total_entries: entries.length, file_url: entry.file_url, drive_file_id: entry.drive_file_id, likes: entry.likes_count, views: entry.views_count, user_name: entry.user_name });
        }
      }
  ...
  app.post('/api/admin/weekly/approve-ip', async (req, res) => {
    ...
    try {
      await pool.query('UPDATE weekly_entries SET ip_approved = 1 WHERE topic_id = ? AND user_email = ?', [topicId, userEmail]);
      res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  app.delete('/api/admin/weekly/disqualify', async (req, res) => {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.drive_file_id) await drive.files.delete({ fileId: entry.drive_file_id }).catch(e => console.log(e.message));
  ...
  app.get('/api/weekly/hall-of-fame', async (req, res) => {
    try {
      const currentNow = getLocalMySQLNow();
      const [rows] = await pool.query("SELECT u.name as user_name, u.email as user_email, u.club_name, c.drive_logo_id, c.logo_url, COALESCE(SUM(IF(e.is_active = 1 OR t.end_date < ?, e.likes_count, 0)), 0) as total_likes, (SELECT COUNT(*) FROM weekly_entries e1 WHERE e1.user_email = u.email AND e1.is_active = 1 AND e1.topic_id IN (SELECT id FROM weekly_topics WHERE end_date < ?) AND e1.id = (SELECT e2.id FROM weekly_entries e2 WHERE e2.topic_id = e1.topic_id AND e2.is_active = 1 ORDER BY e2.likes_count DESC, e2.views_count ASC LIMIT 1)) AS first_places, (SELECT COUNT(*) FROM weekly_entries e1 WHERE e1.user_email = u.email AND e1.is_active = 1 AND e1.topic_id IN (SELECT id FROM weekly_topics WHERE end_date < ?) AND (SELECT COUNT(*) FROM weekly_entries e2 WHERE e2.topic_id = e1.topic_id AND e2.is_active = 1 AND (e2.likes_count > e1.likes_count OR (e2.likes_count = e1.likes_count AND e2.views_count < e1.views_count)))) < 3) AS podiums FROM photo_users u LEFT JOIN weekly_entries e ON u.email = e.user_email LEFT JOIN weekly_topics t ON e.topic_id = t.id LEFT JOIN photo_clubs c ON u.club_name = c.name GROUP BY u.email, u.name, u.club_name, c.drive_logo_id, c.logo_url HAVING total_likes > 0 ORDER BY total_likes DESC, u.name ASC", [currentNow, currentNow, currentNow]);
      res.json(rows);
    } catch (err) {
      try {
        const currentNow = getLocalMySQLNow();
        const [fallbackRows] = await pool.query("SELECT u.name as user_name, u.email as user_email, u.club_name, NULL as drive_logo_id, NULL as logo_url, COALESCE(SUM(IF(e.is_active = 1 OR t.end_date < ?, e.likes_count, 0)), 0) as total_likes, (SELECT COUNT(*) FROM weekly_entries e1 WHERE e1.user_email = u.email AND e1.is_active = 1 AND e1.topic_id IN (SELECT id FROM weekly_topics WHERE end_date < ?) AND e1.id = (SELECT e2.id FROM weekly_entries e2 WHERE e2.topic_id = e1.topic_id AND e2.is_active = 1 ORDER BY e2.likes_count DESC, e2.views_count ASC LIMIT 1)) AS first_places, (SELECT COUNT(*) FROM weekly_entries e1 WHERE e1.user_email = u.email AND e1.is_active = 1 AND e1.topic_id IN (SELECT id FROM weekly_topics WHERE end_date < ?) AND (SELECT COUNT(*) FROM weekly_entries e2 WHERE e2.topic_id = e1.topic_id AND e2.is_active = 1 AND (e2.likes_count > e1.likes_count OR (e2.likes_count = e1.likes_count AND e2.views_count < e1.views_count)))) < 3) AS podiums FROM photo_users u LEFT JOIN weekly_entries e ON u.email = e.user_email LEFT JOIN weekly_topics t ON e.topic_id = t.id GROUP BY u.email, u.name, u.club_name HAVING total_likes > 0 ORDER BY total_likes DESC, u.name ASC", [currentNow, currentNow, currentNow]);
        res.json(fallbackRows);
      } catch (fErr) { res.status(500).json({ error: 'Hiba' }); }
    }
  });
  ...
  app.get('/api/weekly/base64-proxy', async (req, res) => {
    const { url } = req.query; if (!url) return res.status(400).json({ error: 'Hiányzó URL' });
    try {
  ...
  app.post('/api/weekly/propose', upload.single('cover'), async (req, res) => {
    ...
    try {
      ...
      if (req.file) {
        ...
        coverUrl = result.secure_url; if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
      }
      await pool.query(`INSERT INTO weekly_topics (title, title_en, description, description_en, cover_url, cover_author, master_email, start_date, end_date, status, proposed_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)`, [title, title_en || null, description, description_en || null, coverUrl, cover_author, master_name || null, start_date, end_date, userEmail]);
      res.json({ success: true });
    } catch (err) { if (req.file && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path); res.status(500).json({ error: err.message }); }
  });
        
  app.get('/api/admin/proposals', async (req, res) => {
    try { const [proposals] = await pool.query("SELECT * FROM weekly_topics WHERE status = 'pending' ORDER BY start_date ASC"); res.json(proposals); } catch (err) { res.status(500).json({ error: 'Hiba' }); }
  });

  app.post('/api/admin/decide-proposal', async (req, res) => {
    const { topicId, decision } = req.body; 
    try { await pool.query("UPDATE weekly_topics SET status = ? WHERE id = ?", [decision, topicId]); res.json({ success: true }); } catch (err) { res.status(500).json({ error: 'Hiba' }); }
  });

  app.get('/api/weekly/archive/comments/:entryId', async (req, res) => {
    try { const [comments] = await pool.query("SELECT id, user_name, user_email, comment_text, created_at FROM weekly_archive_comments WHERE entry_id = ? ORDER BY created_at ASC", [req.params.entryId]); res.json(comments); } catch (err) { res.status(500).json({ error: err.message }); }
  });

  app.post('/api/weekly/archive/comment', async (req, res) => {
    const { entryId, userEmail, userName, commentText } = req.body;
    try { await pool.query("INSERT INTO weekly_archive_comments (entry_id, user_email, user_name, comment_text) VALUES (?, ?, ?, ?)", [entryId, userEmail, userName, commentText.trim()]); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  app.post('/api/admin/decide-master', async (req, res) => {
    const { topicId, decision } = req.body; 
    try {
      if (decision === 'approved') await pool.query('UPDATE weekly_topics SET master_email = pending_master_email, pending_master_email = NULL WHERE id = ?', [topicId]);
      else await pool.query('UPDATE weekly_topics SET pending_master_email = NULL WHERE id = ?', [topicId]);
      res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  app.get('/api/weekly/history/:topicId', async (req, res) => {
    try {
      const [leaderboard] = await pool.query("SELECT e.id, e.topic_id, e.user_email, COALESCE(u.name, e.user_name) as user_name, e.file_url, e.drive_file_id, e.likes_count, e.views_count, u.club_name FROM weekly_entries e LEFT JOIN photo_users u ON e.user_email = u.email WHERE e.topic_id = ? AND e.is_active = 1 ORDER BY e.likes_count DESC, e.views_count ASC", [req.params.topicId]);
  ...
  app.get('/api/weekly/chat/:topicId', async (req, res) => {
    ...
    try {
      const [messages] = await pool.query("SELECT c.id, COALESCE(u.name, c.user_name) AS user_name, c.user_email, c.message_text, c.created_at FROM weekly_chat c LEFT JOIN photo_users u ON c.user_email = u.email WHERE c.topic_id = ? ORDER BY c.created_at ASC LIMIT 100", [parsedTopicId]);
  ...
  app.post('/api/weekly/chat', async (req, res) => {
    ...
    try {
      await pool.query('INSERT INTO weekly_chat (topic_id, user_email, user_name, message_text) VALUES (?, ?, ?, ?)', [Number(topicId), userEmail, userName, messageText.trim()]);
      res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  app.post('/api/weekly/report-off-topic', async (req, res) => {
    try {
      await pool.query('UPDATE weekly_entries SET off_topic_count = off_topic_count + 1 WHERE id = ?', [req.body.entryId]);
      res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
};
