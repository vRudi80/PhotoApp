...
module.exports = function(app, pool, drive, upload, cleanupTempFile) {
  ...
  // 1. JAVÍTVA: TÖBBI PÁRHUZAMOSAN FUTÓ PÁRBAJ ÉS ARENA DATA KEZELÉSE
  app.get('/api/weekly/current', async (req, res) => {
    ...
    try {
      ...
      // Kiszűrjük az ÖSSZES jelenleg futó aktív párbajt
      ...
      // HA NINCS SPECIFIKUS TOPIC_ID: Visszaadjuk az aktív párbajok listáját státuszokkal
      if (!topicId) {
      ...
      // HA VAN SPECIFIKUS TOPIC_ID: Betöltjük annak a konkrét párbajnak az Aréna adatait
  ...
  app.post('/api/weekly/upload', upload.single('photo'), async (req, res) => {
    ...
    try {
      ...
      if (existing.length > 0) { cleanupTempFile(file); return res.status(400).json({ error: 'Már töltöttél fel képet erre a kihívásra!' }); }
      ...
      const driveRes = await drive.files.create({ requestBody: { name: `Parhuzamos_${topicId}_${userName}_${Date.now()}${fileExt}`, parents: [process.env.DRIVE_MASTER_FOLDER_ID] }, media: { mimeType: file.mimetype, body: fileStream }, fields: 'id, webViewLink' });
  ...
  // 3. KÉPCSERE FUNKCIÓ (EGYSZERI LEHETŐSÉG)
  app.post('/api/weekly/swap', upload.single('photo'), async (req, res) => {
    ...
    try {
      ...
      if (existing.length === 0) { cleanupTempFile(file); return res.status(400).json({ error: 'Még nem neveztél erre a kihívásra!' }); }
      if (existing[0].swapped >= 1) { cleanupTempFile(file); return res.status(400).json({ error: 'Ebben a párbajban már elhasználtad az egyetlen képcsere lehetőségedet!' }); }

      if (existing[0].drive_file_id) {
        await drive.files.delete({ fileId: existing[0].drive_file_id }).catch(e => console.log("Drive törlési hiba:", e.message));
      }

      const fileStream = fs.createReadStream(file.path);
      const fileExt = file.originalname && file.originalname.includes('.') ? file.originalname.substring(file.originalname.lastIndexOf('.')).toLowerCase() : '.jpg';
      const driveRes = await drive.files.create({ requestBody: { name: `Parhuzamos_SWAP_${topicId}_${userName}_${Date.now()}${fileExt}`, parents: [process.env.DRIVE_MASTER_FOLDER_ID] }, media: { mimeType: file.mimetype, body: fileStream }, fields: 'id, webViewLink' });
  ...
  app.post('/api/weekly/vote', async (req, res) => {
    ...
    try {
      ...
      await conn.beginTransaction ? await conn.commit() : null;
      res.json({ success: true });
  ...
  app.get('/api/weekly/history/:topicId', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ error: 'Hiba az archívum betöltésekor' }); }
  });
  ...
  app.post('/api/weekly/report-off-topic', async (req, res) => {
    ...
    try {
      ...
      } catch (likeErr) { console.error(likeErr.message); }
      res.json({ success: true });
