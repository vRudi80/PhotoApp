...
app.get('/api/my-entries', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM photo_entries WHERE user_email = ? ORDER BY created_at DESC', [req.query.userEmail]);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: 'Hiba' }); }
});
...
app.get('/api/jury-entries/:contestId', async (req, res) => {
  try {
    const [rows] = await pool.query(`
      ...
      WHERE e.contest_id = ? AND v.id IS NULL
    `, [req.query.userEmail, req.params.contestId]);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: 'Hiba' }); }
});
...
app.post('/api/vote', async (req, res) => {
  ...
  try {
    await pool.query('INSERT INTO photo_votes (entry_id, jury_email, score) VALUES (?, ?, ?)', [entryId, juryEmail, score]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: 'Hiba' }); }
});
...
app.get('/api/results/:contestId', async (req, res) => {
  try {
    const [rows] = await pool.query(`
      ...
      FROM photo_entries e
      LEFT JOIN photo_votes v ON e.id = v.entry_id
      WHERE e.contest_id = ?
      GROUP BY e.id
      ORDER BY e.category ASC, total_score DESC
    `, [req.params.contestId]);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: 'Hiba az eredményeknél' }); }
});
