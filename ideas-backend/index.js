...
// 1. Aktuális téma lekérése (Súlyozott 100 pontos skála: 80% Kép, 20% Aktivitás)
app.get('/api/weekly/current', async (req, res) => {
  ...
  try {
    ...
    // Az alap érvényességhez (lakat levételhez) továbbra is elég 3 szavazat.
    const requiredVotes = Math.max(0, Math.min(3, totalEntries - 1));
    
    // ÚJ: Összes szavazható kép száma a rendszerben (Saját magát kivonva, nullával osztás elkerülve)
    ...
    // JS MATEMATIKA (100 pontos skála)
    const processedLeaderboard = rawLeaderboard.map(entry => {
      ...
      // 1. Fotó Minősége (Max 80 pont)
      const winRate = likes / views; 
      const qualityScore = winRate * 80;

      // 2. Aktivitási Bónusz (Max 20 pont) - Százalékos arányban!
    ...
    res.json({ 
      ...
      requiredVotes,
      votableEntries, // Ezt átküldjük a frontendnek a figyelmeztetéshez!
      leaderboard: processedLeaderboard.slice(0, 15) 
    });
  } catch (err) { 
    console.error(err);
...
app.post('/api/weekly/upload', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    const driveRes = await drive.files.create({ 
      requestBody: { name: `Heti_${topicId}_${userName}_${Date.now()}${fileExt}`, parents: [process.env.DRIVE_MASTER_FOLDER_ID] }, 
      media: { mimeType: file.mimetype, body: fileStream }, 
      fields: 'id, webViewLink' 
    });
    ...
    await pool.query(
      'INSERT INTO weekly_entries (topic_id, user_email, user_name, file_url, drive_file_id) VALUES (?, ?, ?, ?, ?)', 
      [topicId, userEmail, userName, driveRes.data.webViewLink, driveRes.data.id]
    );
...
app.get('/api/weekly/next-vote', async (req, res) => {
  ...
  try {
    // Keresünk olyan képet az adott témában, ami NEM a miénk, és MÉG NEM szavaztunk rá.
    // Trükk: Növekvő views_count szerint rendezzük (hogy az újakat dobja fel először), 
    // abból kiveszünk 10-et, és abból választunk egyet véletlenszerűen, hogy ne legyen kiszámítható.
    const [candidates] = await pool.query(`
      SELECT e.id, e.file_url, e.drive_file_id 
      FROM weekly_entries e
      LEFT JOIN weekly_votes v ON e.id = v.entry_id AND v.voter_email = ?
      WHERE e.topic_id = ? 
        AND e.user_email != ? 
        AND v.id IS NULL
      ORDER BY e.views_count ASC
      LIMIT 10
    `, [userEmail, topicId, userEmail]);

    if (candidates.length === 0) {
      return res.json({ entry: null, message: 'Minden e-heti képet értékeltél már!' });
    }

    // Random választunk a legkevesebbet látott 10 kép közül
...
app.post('/api/weekly/vote', async (req, res) => {
  const { entryId, userEmail, voteType } = req.body; // voteType: 'like' vagy 'pass'
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    
    // Megnézzük, nem szavazott-e már (duplikáció védelem)
    const [existing] = await conn.query('SELECT id FROM weekly_votes WHERE entry_id = ? AND voter_email = ?', [entryId, userEmail]);
    if (existing.length > 0) {
      await conn.rollback();
      return res.json({ success: false, message: 'Már szavaztál erre a képre!' });
    }

    // Szavazat rögzítése
    await conn.query('INSERT INTO weekly_votes (entry_id, voter_email, vote_type) VALUES (?, ?, ?)', [entryId, userEmail, voteType]);

    // Kép statisztikájának frissítése (Megtekintés +1, és ha like, akkor Like +1)
    if (voteType === 'like') {
      await conn.query('UPDATE weekly_entries SET views_count = views_count + 1, likes_count = likes_count + 1 WHERE id = ?', [entryId]);
    } else {
      await conn.query('UPDATE weekly_entries SET views_count = views_count + 1 WHERE id = ?', [entryId]);
    }
    ...
    await conn.rollback(); 
    res.status(500).json({ error: 'Hiba a szavazás rögzítésekor' }); 
  } finally { 
    conn.release(); 
  }
});
...
// ==========================================
// KISZERVEZETT ÚTVONALAK (ROUTES) BEKÖTÉSE
// ==========================================
