...
// 🎯 KÖZPONTI SEGÉDFÜGGVÉNY: Legyártja az érvényes biztonsági fejlécet az elmentett tokenből
...
function MainContent() {
  ...
  const handleCreateContest = async () => { 
    ...
    if (user.email === ADMIN_EMAIL) {
      ...
      finalRestrictedClubId = currentDbUser?.club_id;
    }
    ...
    const res = await fetch(`${BACKEND_URL}/api/contests`, { 
      ...
      headers: getAuthHeaders({ 'Content-Type': 'application/json' }), 
      body: JSON.stringify({ 
        title: newTitle, description: newDesc, startDate: newStart, endDate: newEnd, categories: newCats, 
        restrictedClubId: finalRestrictedClubId, sponsorClubId: newSponsorClub ? Number(newSponsorClub) : null, 
        entryFee: newEntryFee, feeCurrency: newFeeCurrency, categorySettings: newCategorySettings 
      }) 
    }); 
    if (res.ok) { setNewTitle(''); setNewDesc(''); setNewStart(''); setNewEnd(''); setNewCats(''); setNewRestrictedClub(''); setNewSponsorClub(''); fetchData(); alert("Pályázat sikeresen kiírva!"); } 
  };
  ...
  const handleUpdateContest = async () => { 
    const res = await fetch(`${BACKEND_URL}/api/contests/${editContestId}`, { 
      ...
      headers: getAuthHeaders({ 'Content-Type': 'application/json' }), 
      body: JSON.stringify({ 
        title: editTitle, description: editDesc, startDate: editStart || null, endDate: editEnd || null, categories: editCats, 
        ...
        entryFee: editEntryFee, feeCurrency: editFeeCurrency, categorySettings: editCategorySettings 
      }) 
    }); 
  ...
  const handleUpload = async (contestId: number) => { 
    ...
    try { 
      ...
      // 🎯 FormDatánál a Content-Type-ot NEM adjuk meg manuálisan, hogy a böngésző maga fűzhesse be a boundary-t!
  ...
  const handleApplyJuryScore = async () => {
    ...
    const res = await fetch(`${BACKEND_URL}/api/vote`, {
      ...
      body: JSON.stringify({ entryId: currentEntry.id, juryEmail: user.email, score: currentScore })
    });
    if (res.ok) {
