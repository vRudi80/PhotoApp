...
if (typeof window !== 'undefined' && window.location.hostname.includes('kepolvasok.guru')) {
  window.location.replace('https://photawesome.com' + window.location.pathname + window.location.search);
}

// 🎯 KÖZPONTI AUTH FEJLÉC GENERÁTOR
...
function MainContent() {
  ...
  // 🏛️ KÖZPONTI ADATLEKÉRŐ MOTOR (Csak érvényes tokennel futhat!)
  ...
  const fetchMyEntries = async (email: string) => {
    if (!email?.trim() || email === 'undefined') return;
    try {
  ...
  // ⚡ LÉPÉS 1: AUTHENTIKÁCIÓS SZINKRON ÉS INDÍTÁS
  useEffect(() => {
    ...
    if (storedToken) {
        ...
        if (decoded.exp * 1000 < Date.now()) {
          ...
          const attemptSync = async (retry = 0) => {
            try {
              ...
              if (!res.ok) throw new Error("Sync hiba");
              const data = await res.json();
              setUser({ 
                ...decoded, 
                isPremium: data.isPremium, is_premium: data.isPremium,
                premiumUntil: data.premiumUntil, premiumLevel: data.premiumLevel, premium_level: data.premiumLevel
              });
              setIsAuthLoading(false);
              
              // 🎯 KRITIKUS PONT: Csak a sikeres beléptetés után indítjuk el a hálózati adatletöltést!
              fetchData();
      ...
      setIsAuthLoading(false);
      setIsInitialLoading(false); // Nincs token, leállítjuk a töltést, jöhet a LoginScreen
    }
  }, []);

  // ⚡ LÉPÉS 2: CSENDES HÁTTÉR-SZINKRONIZÁCIÓ (Csak ha van érvényes token)
  useEffect(() => {
    const silentAuthSync = async () => {
      const storedToken = localStorage.getItem('photoAppToken');
      if (!storedToken) return; // 🔒 Biztonsági fék: ha nincs bejelentkezve, nem hívogatunk a háttérben
      ...
      try {
        ...
        if (authRes.ok) {
          ...
          setUser((prev: any) => ({
            ...prev,
            isPremium: authData.isPremium, is_premium: authData.isPremium,
            premiumLevel: authData.premiumLevel, premium_level: authData.premiumLevel,
            premiumUntil: authData.premiumUntil
          }));
        }

        const usersRes = await fetch(`${BACKEND_URL}/api/users`, { headers: getAuthHeaders() });
        if (usersRes.ok) setAllUsers(await usersRes.json());
      } catch (error) {
        console.error('Csendes szinkronizációs hiba:', error);
      }
    };
  ...
  const handlePayContestFee = async (contestId: number) => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/create-contest-payment`, { method: 'POST', headers: getAuthHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ userEmail: user.email, contestId, returnUrl: window.location.origin }) });
      const data = await res.json();
  ...
  const handleDeleteClub = async (id: number) => { if (!window.confirm("Biztosan törlöd?")) return; const res = await fetch(`${BACKEND_URL}/api/clubs/${id}`, { method: 'DELETE', headers: getAuthHeaders() }); if (res.ok) fetchData(); };
  ...
  const handleUpdateClub = async (id: number, name: string) => {
    try {
      ...
      if (res.ok) { fetchData(); alert('Sikeres frissítés!'); }
    } catch (e) { alert('Hiba!'); }
  };
  ...
  const saveUserClub = async (email: string) => { 
    ...
    const clubId = clubs.find(c => c.name === clubName)?.id || null;
  ...
  const handleCreateContest = async () => { 
    if (!newTitle || !newStart || !newEnd || !newCats) return alert("Minden mező kötelező!"); 
    const res = await fetch(`${BACKEND_URL}/api/contests`, { method: 'POST', headers: getAuthHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ title: newTitle, description: newDesc, startDate: newStart, endDate: newEnd, categories: newCats, restrictedClubId: user.email === ADMIN_EMAIL ? (newRestrictedClub ? Number(newRestrictedClub) : null) : currentDbUser?.club_id, sponsorClubId: newSponsorClub ? Number(newSponsorClub) : null, entryFee: newEntryFee, feeCurrency: newFeeCurrency, categorySettings: newCategorySettings }) }); 
    if (res.ok) { setNewTitle(''); setNewDesc(''); setNewStart(''); setNewEnd(''); setNewCats(''); fetchData(); alert("Sikeres mentés!"); } 
  };
  ...
  const startEdit = (contest: any) => { 
    ...
    const formatDate = (d: string | null) => d ? d.replace('Z', '').substring(0, 16) : '';
  ...
  const handleUpdateContest = async () => { 
    ...
    if (res.ok) { setEditContestId(null); setEditSponsorClub(''); fetchData(); alert("Sikeres frissítés!"); } 
  };
  ...
  const handleUpload = async (contestId: number) => { 
    ...
    try { 
      ...
      if (res.ok) { alert("Sikeres feltöltés!"); setActiveUploadContest(null); setUploadFile(null); setUploadPreview(null); setUploadTitle(''); setUploadCategory(''); fetchMyEntries(user.email); } 
    } catch (error) { alert("Hiba történt!"); } finally { setIsUploading(false); } 
  };

  const handleUpdateEntryTitle = async (entryId: number) => { 
    if (!editEntryTitle) return alert('Nem lehet üres!'); 
  ...
  const handleDeleteContestEntry = async (entryId: number) => {
    ...
    const res = await fetch(`${BACKEND_URL}/api/entries/${entryId}`, { method: 'DELETE', headers: getAuthHeaders() });
    if (res.ok) fetchMyEntries(user.email);
  };

  const handleApplyJuryScore = async () => {
    if (currentScore === '') return;
    const res = await fetch(`${BACKEND_URL}/api/vote`, { method: 'POST', headers: getAuthHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ entryId: unvotedEntries[0].id, juryEmail: user.email, score: currentScore }) });
  ...
  const loadResults = async (contestId: number) => { const res = await fetch(`${BACKEND_URL}/api/results/${contestId}`, { headers: getAuthHeaders() }); if (res.ok) setContestResults(await res.json()); };
  const loadStats = async (contestId: number) => { const res = await fetch(`${BACKEND_URL}/api/admin/stats/${contestId}`, { headers: getAuthHeaders() }); if (res.ok) setContestStats(await res.json()); };
  const handleDeleteContest = async (id: number) => { if (!window.confirm("Biztos törlöd?")) return; const res = await fetch(`${BACKEND_URL}/api/contests/${id}`, { method: 'DELETE', headers: getAuthHeaders() }); if (res.ok) fetchData(); };
  const loadJuryProgress = async (contestId: number) => { const res = await fetch(`${BACKEND_URL}/api/admin/jury-stats/${contestId}`, { headers: getAuthHeaders() }); if (res.ok) setJuryProgressData(await res.json()); };
  ...
  const filteredContests = Array.isArray(contests) ? contests.filter(contest => {
    ...
    const now = new Date(); const end = contest.end_date ? new Date(contest.end_date) : new Date(0); const isEnded = now > end;
    ...
    if (activeTab === 'contests_closed') return isEnded && (!isRestricted || contest.restricted_club === currentDbUser?.club_name || isUserJuryForThisContest);
  ...
  const searchedMeetings = myClubMeetings.filter(m => !meetingSearch || m.topic.toLowerCase().includes(meetingSearch.toLowerCase()) || m.description?.toLowerCase().includes(meetingSearch.toLowerCase()));
  const adminMeetings = user?.email === ADMIN_EMAIL ? meetings : myClubMeetings;
  const myClubHomeworks = homeworks.filter(h => h.club_name === currentDbUser?.club_name);
  const adminHomeworks = user?.email === ADMIN_EMAIL ? homeworks : myClubHomeworks;
  ...
  return (
    <>
      {fullscreenData && <FullscreenModal data={fullscreenData} onClose={() => setFullscreenData(null)} entryList={fullscreenData._entryList} currentIndex={fullscreenData._currentIndex} onNavigate={fullscreenData._onNavigate} onToggleLike={fullscreenData._onToggleLike} />}
      ...
      {showSplash ? (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: '#090d16', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 999999, opacity: animateOut ? 0 : 1, transition: 'opacity 0.6s ease', pointerEvents: 'none' }}>
          <div style={{ width: '90%', maxWidth: '580px', aspectRatio: '16/9', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 25px 60px rgba(0,0,0,0.7)', border: '1px solid rgba(255,255,255,0.06)', background: '#000' }}>
          ...
          <div style={{ marginTop: '30px', textAlign: 'center', animation: 'appSplashPulse 1.8s infinite' }}>
            <h4 style={{ color: '#f8fafc', fontSize: '1.25rem', fontWeight: 'bold', margin: '0 0 6px 0' }}>{lang === 'en' ? 'Launching System...' : 'Rendszer indítása...'}</h4>
            <p style={{ color: '#64748b', fontSize: '0.85rem', margin: 0 }}>{lang === 'en' ? 'Synchronizing files and secure sessions...' : 'Adatok és biztonságos munkamenetek szinkronizálása'}</p>
          </div>
        </div>
      ) : null}
