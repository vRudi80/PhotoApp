...
function App() {
  ...
  // --- ÚJ: AUTOKMATIKUS BELÉPTETÉS OLDALFRISSÍTÉSKOR ---
  useEffect(() => {
    fetchData();
    ...
    if (storedToken) {
      try {
        ...
        // Ellenőrizzük, hogy a token nem járt-e még le (a Google tokenek kb. 1 óráig élnek a frontenden)
  ...
  const handleLoginSuccess = async (credential: string) => {
    // ELMENTJÜK A TOKENT HOSSZÚTÁVRA
        ...
        <header style={{ padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#1e293b', borderBottom: '1px solid #334155', position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          {user && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              ...
              {/* KIJELENTKEZÉS GOMB (TÖRLI A LEMENTETT TOKENT IS) */}
