...
interface AdItem {
  ...
  is_active?: number; // 👈 Új mező az aktív státuszhoz
  isActive?: number;  // 👈 CamelCase verzió biztonságból
}
    ...
    <div style={{ animation: 'fadeIn 0.4s ease-out', maxWidth: '1200px', margin: '0 auto', color: '#f8fafc', padding: '0 20px' }}>
      ...
      {loading ? (
        ...
        <div className="market-grid">
          {filteredAds.map((ad) => {
            // Ellenőrizzük, hogy eladott-e a hirdetés
            const isSold = ad.is_active === 0 || ad.isActive === 0;

            return (
              <div 
                key={ad.id || ad._id} 
                className="market-card" 
                onClick={() => onNavigateToDetails(ad.id || ad._id)} 
                style={{ 
                  cursor: 'pointer',
                  position: 'relative',
                  // 👈 Kiszürkítés és áttetszőség, ha eladták
              ...
              >
                <div className="card-image-wrapper">
                  {/* 👈 PIROS ELADVA BADGE */}
                  {isSold && (
                    <span style={{ position: 'absolute', top: '12px', left: '12px', background: '#f43f5e', color: 'white', padding: '5px 12px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 'bold', zIndex: 10, boxShadow: '0 4px 10px rgba(244,63,94,0.4)' }}>
                      🚫 ELADVA
                    </span>
                  )}
