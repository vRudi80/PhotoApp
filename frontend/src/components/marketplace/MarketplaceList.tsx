...
interface MarketplaceListProps {
  user: { email: string };
  onNavigateToCreate: () => void; // Navigációs prop a szülőtől
}

export default function MarketplaceList({ user, onNavigateToCreate }: MarketplaceListProps) {
    ...
    <div style={{ animation: 'fadeIn 0.4s ease-out', maxWidth: '1200px', margin: '0 auto', color: '#f8fafc', padding: '0 20px' }}>
      ...
      {loading ? (
        ...
        <div className="market-grid">
          {filteredAds.map((ad) => (
            <div key={ad._id} className="market-card">
              ...
              <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '10px', flexGrow: 1 }}>
                <span style={{ fontSize: '0.8rem', textTransform: 'uppercase', color: '#38bdf8', fontWeight: 'bold', tracking: '1px' }}>
      ...
      <style>{`
        ...
        .market-search-input:focus, .market-filter-select:focus {
          outline: none;
          border-color: #38bdf8;
        }
