...
export default function ArenaActiveRoom({
  ...
  // --- LOCAL STATE A RENDELKEZÉSRE ÁLLÓ ÉLŐ CHATHEZ ---
  const [messages, setMessages] = useState<any[]>([]);
  const [typedMessage, setTypedMessage] = useState('');
  const [isSendingMsg, setIsSendingMsg] = useState(false);
  const chatBottomRef = useRef<HTMLDivElement>(null);

  // Védelmi mentések tömbökhöz
  ...
  // 🔄 CHAT MESSAGES POLING & AUTO-SCROLL MOTOR
  useEffect(() => {
    if (!topic?.id) return;

    const fetchChat = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/api/weekly/chat/${topic.id}`);
        if (res.ok) {
          const data = await res.json();
          setMessages(data || []);
        }
      } catch (err) {
        console.error("Hiba a chat lekérésekor:", err);
      }
    };

    fetchChat();
    const interval = setInterval(fetchChat, 4000); // 4 másodpercenként csendben frissít az élő élményért
    return () => clearInterval(interval);
  }, [topic?.id]);

  // Görgetés az új üzenet aljára
  useEffect(() => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!typedMessage.trim() || !topic?.id || isSendingMsg) return;
    ...
    try {
      ...
      if (res.ok) {
        setTypedMessage('');
        // Azonnali lokális append az azonnali visszacsatolásért
        setMessages(prev => [...prev, { ...msgPayload, created_at: new Date().toISOString() }]);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px', width: '100%' }}>
      ...
      {/* FŐ OLDALRÁCS (Pultok és Toplisták) */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
          ...
          <div style={{ background: '#1e293b', padding: '25px', borderRadius: '24px', border: '1px solid #334155', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
            ...
            {isMaster ? (
              ...
              <div>
                <div style={{ width: '100%', boxSizing: 'border-box', background: '#0f172a', padding: '20px', borderRadius: '16px', border: '1px dashed #38bdf8' }}>
      ...
      {/* ── 💬 ➕ ÚJ SECTION: LÍGA ÉLŐ CSEVEGŐ (FULL-WIDTH CHAT ROOM) ── */}
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '24px', padding: '25px', boxShadow: '0 15px 35px rgba(0,0,0,0.4)', display: 'flex', flexDirection: 'column', gap: '15px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            <h3 style={{ margin: 0, color: '#38bdf8', fontSize: '1.4rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>💬</span> Élő Diskurzus a témáról
            </h3>
            <p style={{ margin: '3px 0 0 0', color: '#94a3b8', fontSize: '0.85rem' }}>Beszélgess a többiekkel, adj tanácsokat, ösztönözd a klubot!</p>
          </div>
          {/* 🔥 Motivációs sáv a meghívókhoz */}
          <div style={{ background: '#38bdf815', border: '1px dashed #38bdf850', color: '#38bdf8', padding: '6px 14px', borderRadius: '10px', fontSize: '0.8rem', fontWeight: 'bold' }}>
            💡 Kevés a fotós? Küldd el a meghívó kódod egy ismerősödnek +10 Jokerért!
          </div>
        </div>

        {/* Üzenőfal görgető konténer */}
        <div style={{ background: '#0f172a', borderRadius: '16px', padding: '20px', height: '280px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px', border: '1px solid #223147' }}>
          {messages.length === 0 ? (
            <div style={{ color: '#475569', textAlign: 'center', margin: 'auto', fontStyle: 'italic', fontSize: '0.9rem' }}>
              Néma a képtár... 🤫 Legyél te az első, aki feldobja a hangulatot egy üzenettel!
            </div>
          ) : (
        ...
        <form onSubmit={handleSendMessage} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <input 
            type="text" 
            placeholder="Szólítsd meg a bent lévőket! Írj ide valamit..." 
            value={typedMessage}
          ...
          <button 
            type="submit"
            disabled={!typedMessage.trim() || isSendingMsg}
            style={{ background: (!typedMessage.trim() || isSendingMsg) ? '#334155' : 'linear-gradient(135deg, #0ea5e9, #2563eb)', color: (!typedMessage.trim() || isSendingMsg) ? '#64748b' : 'white', border: 'none', padding: '14px 28px', borderRadius: '14px', fontWeight: 'bold', fontSize: '0.95rem', cursor: 'pointer', transition: 'all 0.2s', boxShadow: typedMessage.trim() ? '0 4px 15px rgba(37,99,235,0.3)' : 'none' }}
          >
            {isSendingMsg ? '...' : 'Küldés 🚀'}
          </button>
        </form>
