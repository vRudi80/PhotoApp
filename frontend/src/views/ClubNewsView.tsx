...
export default function ClubNewsView({ user, currentDbUser, mode = 'club' }: ClubNewsViewProps) {
  // ==============================================================
  // 1. GLOBÁLIS HELYI ÁLLAPOTOK (Minden Hook szabályosan legfelül)
  // ==============================================================
  ...
  // ==============================================================
  // 2. HÁTTÉR SZINKRONIZÁCIÓK BIZTONSÁGOSAN
  // ==============================================================
  useEffect(() => {
    ...
    const fetchClubId = async () => {
      ...
      try {
        ...
        const res = await fetch(`${BACKEND_URL}/api/clubs`, { headers: getAuthHeaders() });
        if (res.ok) {
          const clubs = await res.json();
          if (Array.isArray(clubs)) { // 🛡️ Biztonsági tömb ellenőrzés
  ...
  const fetchNews = async () => {
    try {
      if (mode === 'public') {
        // 🎯 JAVÍTVA: Hitelesített fejléc beillesztve
        const res = await fetch(`${BACKEND_URL}/api/news/public?userEmail=${user.email}`, { headers: getAuthHeaders() });
        if (res.ok) {
      ...
      // 🎯 JAVÍTVA: Hitelesített fejléc beillesztve
      const res = await fetch(`${BACKEND_URL}/api/clubs/${clubId}/news?userEmail=${user.email}`, { headers: getAuthHeaders() });
      if (res.ok) {
        const data = await res.json();
        setNewsList(Array.isArray(data) ? data : []);
      }
  ...
  // ==============================================================
  // 3. INTERAKTÍV MŰVELETEK VÉDELME
  // ==============================================================
  const handleExpandNews = async (newsId: number) => {
    ...
    try {
      // 🎯 JAVÍTVA: Olvasottság és kommentek lekérése hitelesítve
      await fetch(`${BACKEND_URL}/api/news/${newsId}/read`, {
        method: 'POST',
        headers: getAuthHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ userEmail: user.email })
      });
      const res = await fetch(`${BACKEND_URL}/api/news/${newsId}/comments`, { headers: getAuthHeaders() });
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchReaders = async (newsId: number) => {
    ...
    try {
      // 🎯 JAVÍTVA: Olvasólista lekérése hitelesítve
      const res = await fetch(`${BACKEND_URL}/api/news/${newsId}/readers`, { headers: getAuthHeaders() });
      if (res.ok) {
        const data = await res.json();
  ...
  const handlePostNews = async () => {
    ...
    try {
      // 🎯 JAVÍTVA: Hír beküldése hitelesített fejléccel lezárva
  ...
  const handleDeleteNews = async (newsId: number) => {
    ...
    try {
      // 🎯 JAVÍTVA: Törlés hitelesített fejléccel ellátva
  ...
  const handlePostComment = async (newsId: number) => {
    ...
    try {
      // 🎯 JAVÍTVA: Kommentelés hitelesített fejléccel ellátva
      ...
      if (res.ok) {
        setNewComment('');
        const cRes = await fetch(`${BACKEND_URL}/api/news/${newsId}/comments`, { headers: getAuthHeaders() });
        if (cRes.ok) {
  ...
  // ==============================================================
  // 4. BIZTONSÁGI RENDERING SZŰRŐ (React-szabályos helyen, a Hookok után)
  // ==============================================================
                ...
                >
                  <div style={{ flex: 1 }}>
                    <div style={{ color: '#94a3b8', fontSize: '0.8rem', marginBottom: '5px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                      {isUnread && (
                        <span style={{ background: '#ef4444', color: 'white', padding: '2px 8px', borderRadius: '12px', fontSize: '0.7_rem', fontWeight: 'bold', letterSpacing: '0.5px' }}>
                          ÚJ HÍR
                        </span>
                      )}
                      {news.is_public === 1 && (
