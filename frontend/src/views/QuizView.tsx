...
type Phase = 'intro' | 'loading' | 'playing' | 'revealing' | 'ended' | 'already_played';
...
export default function QuizView({ user }: { user: any }) {
  ...
  const [rewardData, setRewardData] = useState<{ pointsAwarded: number; score: number } | null>(null);

  // Ez a ref tartja a kérdés lejárati időpontját (Date.now() + 20000ms).
  // Timestamp alapján számolunk vissza, NEM egyszerű setInterval-lánccal,
  // mert mobil Safari háttérbe kerüléskor / lomha renderelésnél
  // eldobálhat tickeket, és a sima "1 másodpercenkénti -1" könnyen
  // elcsúszik vagy teljesen leáll.
  const deadlineRef = useRef<number>(0);

  // Szinkron zár: amíg ez true, handleAnswer semmit nem csinál.
  // Azért ref és nem state, mert state csak a következő renderre frissül -
  // ha a kattintás és az időzítő lejárta majdnem egyszerre történik,
  // egy state-alapú védelem mindkettőt átengedné.
  ...
  const parsedQuestion = useMemo(() => {
    ...
    try {
      ...
      console.error('Kérdés opciók parzolási hiba', e);
    }
  ...
  // ── Válasz kezelése (kattintás VAGY időtúllépés) ──
  const handleAnswer = useCallback((optionLetter: string | null) => {
    ...
    setScore(prevScore => {
      ...
      const isCorrect = optionLetter !== null && optionLetter === correct;
      return prevScore + (isCorrect ? 100 : 0);
    });
  }, [currentIdx]);

  // ── Timer: 250ms-enként ellenőrzi a hátralévő időt a deadline alapján ──
  useEffect(() => {
    if (phase !== 'playing') return;

    deadlineRef.current = Date.now() + QUESTION_TIME_MS;
    setTimeLeft(Math.ceil(QUESTION_TIME_MS / 1000));
  ...
  }, [phase, currentIdx, handleAnswer]);

  // ── Kérdésváltás / befejezés a "revealing" fázis után ──
  useEffect(() => {
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      ...
      if (loadedQuestions.length === 0) {
        alert('Jelenleg nincs elérhető kérdés.');
        setPhase('intro');
        return;
      }
    ...
    setScore(currentScore => {
      (async () => {
        try {
          ...
          console.error(e);
          alert('Hálózati hiba történt a beküldéskor.');
          setPhase('intro');
        }
      })();
      return currentScore;
  ...
  return (
    <div style={{ width: '100%', maxWidth: '800px', margin: '0 auto', boxSizing: 'border-box', padding: '10px' }}>
      ...
      {/* ── C: JÁTÉKTÉR (playing + revealing) ── */}
      {(phase === 'playing' || phase === 'revealing') && parsedQuestion && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-card)', padding: '12px 20px', borderRadius: '8px', border: '1px solid var(--border-main)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: timeLeft <= 5 ? '#ef4444' : '#10b981', fontSize: '0.9rem', fontWeight: 'bold' }}>
              <Timer size={14} /> <span>{phase === 'playing' ? timeLeft : 0}s</span>
            </div>
          </div>

          <div style={{ width: '100%', height: '4px', background: '#1e293b', borderRadius: '2px', overflow: 'hidden' }}>
            <div style={{ width: `${((phase === 'playing' ? timeLeft : 0) / 20) * 100}%`, height: '100%', background: timeLeft <= 5 ? '#ef4444' : 'linear-gradient(90deg, #38bdf8, #10b981)', transition: 'width 0.2s linear' }} />
          </div>
            ...
            <div style={{ width: '100%', height: '260px', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid var(--border-main)' }}>
              <img
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src = `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 400 300' fill='%230f172a'><rect width='100%25' height='100%25'/><text x='50%25' y='50%25' fill='%23334155' font-family='sans-serif' font-size='14' text-anchor='middle'>📸 PhotAwesome Arena Quiz</text></svg>`;
                }}
              />
            </div>
                ...
                {['A', 'B', 'C', 'D'].map((letter, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 18px', background: btnBg, border: `1px solid ${btnBorder}`, borderRadius: '8px', color: textColor, fontSize: '0.92rem', fontWeight: '600', cursor: isRevealing ? 'not-allowed' : 'pointer', transition: 'all 0.15s ease' }}
                    >
                      <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '26px', height: '26px', borderRadius: '6px', background: badgeBg, color: 'white', fontSize: '0.8rem', fontWeight: 'bold' }}>
                        {letter}
                      </span>
      ...
      {phase === 'ended' && rewardData && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid #fbbf24', padding: '40px 30px', borderRadius: '12px', textAlign: 'center', boxShadow: '0px 10px 30px rgba(251,191,36,0.15)' }}>
          ...
          <button onClick={() => setPhase('intro')} style={{ width: '100%', background: 'transparent', border: '1px solid var(--border-main)', color: 'var(--text-title)', padding: '12px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.15s' }}>
