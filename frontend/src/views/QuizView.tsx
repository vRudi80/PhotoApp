...
// 🎯 TISZTA FÁZIS-ALAPÚ ÁLLAPOTGÉP DEKLARÁCIÓ
type QuizPhase = 'INTRO' | 'LOADING' | 'QUESTION' | 'FEEDBACK' | 'SUMMARY' | 'ALREADY_PLAYED';

export default function QuizView({ user }: { user: any }) {
  const { lang, t } = useLanguage();
  
  // Központi State Machine kapu
  const [phase, setPhase] = useState<QuizPhase>('INTRO');
  const [questions, setQuestions] = useState<any[]>([]);
  const [currentIdx, setCurrentIdx] = useState(0);
  
  // Tiszta játékmenet adatok
  ...
  // 🎯 IDŐBÉLYEG REFERENCIA PAJZS: Ide mentjük a lejárati Unix időbélyeget
  const questionEndTimeRef = useRef<number>(0);

  // 📡 1. JÁTÉK INDÍTÁSA (Kérdések lehúzása)
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.alreadyPlayed) {
          ...
          // Beállítjuk az első céldátumot: Most + 20 másodperc
          questionEndTimeRef.current = Date.now() + 20000;
          setTimeLeft(20);
          setPhase('QUESTION');
        }
      } else {
  ...
  // Adatvédelmi parser a válaszlehetőségekhez
  const parsedQuestion = useMemo(() => {
    ...
    try {
      ...
      console.error("Kvíz JSON parse hiba", e);
    }
  ...
  // 🎯 2. GOLYÓÁLLÓ IDŐBÉLYEG ALAPÚ VISSZASZÁMLÁLÓ MOTOR
  // Kiváló precizitású loop, ami kiküszöböli a React renderelési driftjeit
  useEffect(() => {
    if (phase !== 'QUESTION') return;

    // Minden kérdésváltásnál újra-kalibráljuk a fix végpontot
    questionEndTimeRef.current = Date.now() + 20000;
    setTimeLeft(20);

    const interval = setInterval(() => {
      const now = Date.now();
      // Kiszámoljuk a hátralévő másodperceket a céldátum és a jelenlegi idő különbségéből
      const remainingSeconds = Math.max(0, Math.ceil((questionEndTimeRef.current - now) / 1000));
      
      setTimeLeft(remainingSeconds);

      // Időtúllépési trigger
      if (remainingSeconds <= 0) {
        clearInterval(interval);
        handleAnswer(''); // Automatikus továbblépés üres stringgel
      }
    }, 200); // 200 ms-enként frissítünk a tökéletesen sima csík-haladásért

    return () => clearInterval(interval);
  }, [phase, currentIdx]);

  // 📡 3. KVÍZ ZÁRÁSA ÉS PONTMENTÉS
  const handleQuizFinished = async (finalScore: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Hiba történt az eredmények mentésekor.");
        setPhase('INTRO');
      }
    } catch (e) {
      console.error(e);
  ...
  // 🎮 4. KÖZPONTI FÁZIS-ORIENTÁLT VÁLASZKEZELŐ ( deadlock mentes )
  const handleAnswer = (optionLetter: string) => {
    if (phase !== 'QUESTION') return;

    // Azonnal átváltunk FEEDBACK fázisra, így a gombok azonnal zárolódnak
    setPhase('FEEDBACK');
    ...
    // 1.5 másodperces villanás után dönti el a fázisgép a köv lépést (akár 1 kérdésnél is stabil)
    setTimeout(() => {
      const nextIndex = currentIdx + 1;
      if (nextIndex < questions.length) {
        setSelectedOption(null);
        setCurrentIdx(nextIndex);
        setPhase('QUESTION'); // Visszalépünk kérdés fázisba -> Újraindul a fenti céldátum óra!
      } else {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '800px', margin: '0 auto', boxSizing: 'border-box', padding: '10px' }}>
      ...
      {/* ── B: CSERÉLHETŐ TÖLTŐSÁV PANEL ── */}
      {phase === 'LOADING' && (
        <div style={{ padding: '60px 0', textAlign: 'center' }}>
          <VideoLoader />
          <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginTop: '15px', animation: 'arenaPulse 2s infinite' }}>
            {lang === 'en' ? 'Processing Action...' : 'Művelet feldolgozása...'}
          </p>
        </div>
      )}

      {/* ── C: AKTÍV JÁTÉKTÉR (QUESTION & FEEDBACK FÁZISOK) ── */}
      {(phase === 'QUESTION' || phase === 'FEEDBACK') && parsedQuestion && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ width: '100%', height: '4px', background: '#1e293b', borderRadius: '2px', overflow: 'hidden' }}>
            {/* 🎯 Időcsík csúszka reaktív szélesség-számítása */}
            <div style={{ width: `${(timeLeft / 20) * 100}%`, height: '100%', background: timeLeft <= 5 ? '#ef4444' : 'linear-gradient(90deg, #38bdf8, #10b981)', transition: 'width 0.2s linear' }} />
          </div>
            ...
            <div style={{ padding: '24px 20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {['A', 'B', 'C', 'D'].map((letter, i) => {
                  ...
                  // Neon keret-villanások kezelése szigorúan a fázis ellenőrzésével
                  if (phase === 'FEEDBACK') {
                  ...
                  return (
                    <button
                      key={letter}
                      disabled={phase !== 'QUESTION'} // Kizárólag kérdés fázisban kattintható!
                      onClick={() => handleAnswer(letter)}
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 18px', background: btnBg, border: `1px solid ${btnBorder}`, borderRadius: '8px', color: textColor, fontSize: '0.92rem', fontWeight: '600', cursor: phase !== 'QUESTION' ? 'not-allowed' : 'pointer', transition: 'all 0.15s ease' }}
                    >
                      <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '26px', height: '26px', borderRadius: '6px', background: phase === 'FEEDBACK' && letter === parsedQuestion.correct ? '#10b981' : (phase === 'FEEDBACK' && letter === selectedOption ? '#ef4444' : '#1e293b'), color: 'white', fontSize: '0.8rem', fontWeight: 'bold' }}>
                        {letter}
                      </span>
