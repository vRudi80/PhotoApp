...
import { Trophy, Star, Timer, Sparkles, CheckCircle2, XCircle, AlertCircle } from 'lucide-react';
...
export default function QuizView({ user }: { user: any }) {
  ...
  // 🎯 ÚJ: A szervertől kapott biztonságos megoldókulcs tárolója a hibalistához
  ...
  // 🎯 ÓRAJEL JAVÍTVA: Kényszerített lokális trigger-változót kapott, ami átüti a böngésző cache-blokkját
  useEffect(() => {
  ...
  const handleFinalSubmit = async (finalAnswers: Record<number, string>) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setResultData(data);
        setCorrectAnswers(data.correctAnswers || {}); // Elmentjük a kapott megoldókulcsot
        setPhase('SUMMARY');
      } else {
  ...
  return (
    <div style={{ width: '100%', maxWidth: '850px', margin: '0 auto', boxSizing: 'border-box', padding: '10px' }}>
      ...
      {phase === 'LOADING' && (
        <div style={{ padding: '60px 0', textAlign: 'center' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginTop: '15px', animation: 'arenaPulse 2s infinite' }}>
            {lang === 'en' ? 'Analyzing Submissions Securely...' : 'Biztonságos kiértékelés és könyvelés folyamatban...'}
          </p>
        </div>
      )}
            ...
            <div style={{ padding: '24px 20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {['A', 'B', 'C', 'D'].map((letter, i) => {
                  ...
                  return (
                    <button key={letter} onClick={() => handleSelectOption(letter)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 18px', background: '#0f172a50', border: '1px solid var(--border-main)', borderRadius: '8px', color: 'var(--text-body)', fontSize: '0.92rem', fontWeight: '600', cursor: 'pointer' }}>
      ...
      {/* ── D: ÚJ JAVÍTOTT JUTALOM ÉS SZAKMAI ELLENŐRZŐ PANEL ── */}
      {phase === 'SUMMARY' && rewardData && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
          
          {/* Fő kártya tisztázott funkcióleírással */}
          <div style={{ background: 'var(--bg-card)', border: '1px solid #fbbf24', padding: '40px 30px', borderRadius: '12px', textAlign: 'center', boxShadow: '0px 10px 30px rgba(251,191,36,0.15)' }}>
            ...
            <div style={{ background: '#0f172a', padding: '20px', borderRadius: '12px', border: '1px solid #fbbf2450', display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginBottom: '25px', maxWidth: '400px' }}>
              <span style={{ color: '#fbbf24', fontWeight: '900', fontSize: '1.5rem' }}>+{rewardData.pointsAwarded} Levásárolható Aréna Pont</span>
              <span style={{ color: '#10b981', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>
                🟢 SIKERESEN JÓVÁÍRVA A TÁRCÁDBAN!
              </span>
              <p style={{ color: '#64748b', fontSize: '0.78rem', margin: '5px 0 0 0', lineHeight: '1.4' }}>
                Ez a pontszám közvetlenül bekerült a számládra, és azonnal **felhasználható a Boltban Joker cserék vásárlására**, vagy bármilyen egyéb prémium portál funkcióra.
              </p>
            </div>

            <button onClick={() => setPhase('INTRO')} style={{ width: '100%', maxWidth: '300px', margin: '0 auto', background: 'transparent', border: '1px solid var(--border-main)', color: 'var(--text-title)', padding: '12px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>
              {lang === 'en' ? 'Back to Desk' : 'Vissza a pultra'}
            </button>
          </div>

          {/* 🎯 ÚJ SZAKMAI ELLENŐRZŐ PANEL (REVIEW LIST) */}
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-main)', padding: '30px', borderRadius: '12px' }}>
            <h3 style={{ margin: '0 0 20px 0', color: '#f1f5f9', fontSize: '1.2rem', fontWeight: '800', borderBottom: '1px solid var(--border-main)', paddingBottom: '12px' }}>
              🔍 {lang === 'en' ? 'Professional Performance Review' : 'Szakmai Értékelő & Hibajegyzék'}
            </h3>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {questions.map((q, idx) => {
                const userAns = selectedAnswers[q.id] || 'Nincs';
                const correctAns = correctAnswers[q.id] || 'A';
                const isCorrect = String(userAns).toUpperCase() === String(correctAns).toUpperCase();

                // Kikeresjük az opció szövegét
                let optionsArray: string[] = [];
                try { optionsArray = typeof q.options_hu === 'string' ? JSON.parse(q.options_hu) : q.options_hu; } catch(e){}

                return (
                  <div key={q.id} style={{ display: 'flex', gap: '15px', background: '#0f172a50', padding: '15px', borderRadius: '8px', border: `1px solid ${isCorrect ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}`, alignItems: 'center' }}>
                    <div style={{ width: '60px', height: '60px', background: '#000', borderRadius: '6px', overflow: 'hidden', flexShrink: 0 }}>
                      <img src={q.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    
                    <div style={{ flex: 1 }}>
                      <h4 style={{ margin: '0 0 4px 0', color: '#f8fafc', fontSize: '0.9rem', fontWeight: '700' }}>
                        {idx + 1}. {lang === 'en' ? q.question_en : q.question_hu}
                      </h4>
                      <div style={{ display: 'flex', gap: '15px', fontSize: '0.82rem', marginTop: '6px' }}>
                        <span style={{ color: isCorrect ? '#10b981' : '#f87171', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 'bold' }}>
                          {isCorrect ? <CheckCircle2 size={12} /> : <XCircle size={12} />} 
                          {lang === 'en' ? 'Your pick' : 'Te tipped'}: {userAns}
                        </span>
                        {!isCorrect && (
                          <span style={{ color: '#38bdf8', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <AlertCircle size={12} /> {lang === 'en' ? 'Correct' : 'Helyes válasz'}: {correctAns}
                          </span>
                        )}
                      </div>
                    </div>
      ...
      {phase === 'ALREADY_PLAYED' && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid #ef4444', padding: '40px 30px', borderRadius: '12px', textAlign: 'center', boxShadow: '0 10px 25px rgba(239,68,68,0.1)' }}>
          ...
          <p style={{ color: 'var(--text-body)', fontSize: '0.88rem', lineHeight: '1.5', maxWidth: '450px', margin: '0 auto 20px auto' }}>
            {lang === 'en' 
              ? 'The LensMaster Quiz can only be played once a day for points. Come back tomorrow for a fresh set of questions!' 
