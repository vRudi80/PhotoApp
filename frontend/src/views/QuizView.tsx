...
import { Trophy, Star, Timer, Sparkles, CheckCircle2, XCircle, AlertCircle, HelpCircle } from 'lucide-react';
...
export default function QuizView({ user }: { user: any }) {
  ...
  // 🎯 CACHE ÉS REMOUNT BIZTOS IDŐZÍTŐ
  useEffect(() => {
  ...
  const getReadableOptionText = (letter: string, rawOptions: any) => {
    if (!letter || letter === '') return lang === 'en' ? 'No Answer (Timeout)' : 'Nem válaszoltál (Időtúllépés)';
    try {
    ...
    <div style={{ width: '100%', maxWidth: '850px', margin: '0 auto', boxSizing: 'border-box', padding: '10px' }}>
      ...
      {phase === 'INTRO' && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-main)', padding: '40px 30px', borderRadius: '12px', textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.3)' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '0.95rem', lineHeight: '1.6', maxWidth: '500px', margin: '0 auto 20px auto' }}>
      ...
      {phase === 'SUMMARY' && rewardData && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid #fbbf24', padding: '40px 30px', borderRadius: '12px', textAlign: 'center', boxShadow: '0px 10px 30px rgba(251,191,36,0.15)' }}>
            ...
            {/* 🎯 JAVÍTVA: Részletes matematikai eredmény-összetételi panel a levásárolható tisztánlátáshoz */}
            <div style={{ background: '#0f172a', border: '1px solid var(--border-main)', borderRadius: '10px', padding: '18px', maxWidth: '500px', margin: '0 auto 25px auto', textAlign: 'left', fontSize: '0.88rem', lineHeight: '1.6' }}>
              <h4 style={{ margin: '0 0 10px 0', color: '#38bdf8', fontWeight: '800', textTransform: 'uppercase', fontSize: '0.78rem' }}>
                📊 Eredményed részletes összetétele:
              </h4>
              <div style={{ color: '#cbd5e1', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#fbbf24', fontSize: '1rem', fontWeight: 'bold' }}>
                  <span>🪙 Levásárolható egyenleg:</span>
                  <span>+{rewardData.pointsAwarded}p</span>
                </div>
              </div>
            </div>
          ...
          {/* ── SZAKMAI KIÉRTÉKELŐ TUDÁSTÁR MAGYARÁZATOKKAL ── */}
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-main)', padding: '30px', borderRadius: '12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {questions.map((q, idx) => {
                ...
                // Nyelvi magyarázat kinyerése
                const explanationText = lang === 'en' ? q.explanation_en : q.explanation_hu;

                return (
                  <div key={q.id} style={{ background: '#0f172a50', padding: '18px', borderRadius: '10px', border: `1px solid ${isCorrect ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)'}` }}>
                    ...
                    {/* 🎯 JAVÍTVA: Ha az admin írt hozzá edukációs háttérinfót, az stílusosan kinyílik alatta! */}
