...
export default function ClubHomeworksView({
  ...
  if (!currentDbUser?.club_name || currentDbUser?.club_role === 'pending') {
    return (
      <div style={{ textAlign: 'center', padding: '3rem 1.5rem', background: 'var(--bg-card, #1e293b)', borderRadius: '16px', border: '1px solid var(--border-main, #334155)', boxSizing: 'border-box', width: '100%' }}>
        <div style={{ fontSize: '3.5rem', marginBottom: '1rem' }}>🔒</div>
        <h2 style={{ color: isLight ? '#d97706' : '#f59e0b', margin: '0 0 10px 0', wordBreak: 'break-word', fontSize: '1.4rem' }}>
          {currentDbUser?.club_role === 'pending' ? 'Jelentkezésed jóváhagyásra vár' : 'Nincs klubtagságod'}
        </h2>
        <p style={{ color: 'var(--text-body, #94a3b8)', fontSize: '0.95rem', lineHeight: '1.5', wordBreak: 'break-word' }}>
  ...
  return (
    <div style={{ width: '100%', maxWidth: '100vw', overflowX: 'hidden', boxSizing: 'border-box' }}>
      <h2 style={{ fontSize: 'clamp(1.4rem, 4vw, 1.8rem)', marginBottom: '1.2rem', display: 'flex', alignItems: 'center', gap: '10px', wordBreak: 'break-word', color: 'var(--text-title, #f8fafc)' }}>
        <span>📝</span> Házi Feladatok: {currentDbUser.club_name}
      </h2>

      <div style={{ marginBottom: '20px', display: 'flex', gap: '12px', alignItems: 'center', background: 'var(--bg-card, #1e293b)', padding: '12px', borderRadius: '12px', border: '1px solid var(--border-main, #334155)', boxSizing: 'border-box', width: '100%' }}>
        <input type="text" placeholder="🔍 Keresés feladat címében vagy leírásában..." value={hwSearch} onChange={e => setHwSearch(e.target.value)} style={{ flex: 1, padding: '10px 12px', borderRadius: '8px', border: '1px solid var(--border-main, #475569)', background: 'var(--bg-main, #0f172a)', color: 'var(--text-title, white)', outline: 'none', fontSize: '0.88rem', boxSizing: 'border-box', width: '100%' }} />
      </div>

      {filteredHomeworks.length === 0 ? (
        <div style={{ padding: '20px', color: 'var(--text-body, #94a3b8)', textAlign: 'center', background: 'var(--bg-card, #1e293b)', borderRadius: '12px', border: '1px solid var(--border-main, #334155)', fontSize: '0.9rem' }}>
        ...
        filteredHomeworks.map(hw => {
          ...
          return (
            <div key={hw.id} style={{ backgroundColor: 'var(--bg-card, #1e293b)', padding: '16px', borderRadius: '14px', marginBottom: '16px', border: isPast ? '1px solid var(--border-main, #475569)' : '1px solid #10b981', boxShadow: isLight ? '0 4px 6px -1px rgba(0,0,0,0.05)' : '0 4px 6px -1px rgba(0, 0, 0, 0.1)', position: 'relative', transition: 'all 0.3s ease', boxSizing: 'border-box', width: '100%' }}>
              ...
              <div className="contest-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '8px' }}>
                <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                  <h3 style={{ margin: '0 0 4px 0', fontSize: '1.25rem', color: isPast ? 'var(--text-body, #cbd5e1)' : 'var(--text-title, #f8fafc)', wordBreak: 'break-word' }}>{hw.topic}</h3>
                  <p style={{ color: 'var(--text-body, #94a3b8)', fontSize: '0.88rem', margin: '0 0 12px 0', lineHeight: '1.5', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{hw.description}</p>
                </div>
              ...
              <p style={{fontSize: '0.8rem', color: isLight ? '#d97706' : '#f59e0b', margin: '0 0 12px 0', fontWeight: 'bold', wordBreak: 'break-word'}}>
              ...
              <button onClick={() => toggleExpand(hw.id)} style={{ width: '100%', background: isExpanded ? (isLight ? 'var(--hover-overlay, #e2e8f0)' : '#334155') : 'var(--bg-main, #0f172a)', color: isExpanded ? 'var(--text-body, #94a3b8)' : (isLight ? '#0284c7' : '#38bdf8'), border: '1px solid var(--border-main, #334155)', padding: '10px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', fontSize: '0.85rem' }}>
              ...
              {isExpanded && (
                <div style={{ marginTop: '16px', animation: 'fadeIn 0.3s ease-out' }}>
                  ...
                  {isLeader && (
                    <div style={{ marginBottom: '16px', padding: '14px', background: 'var(--bg-main, #0f172a)', borderRadius: '12px', border: '1px solid #f59e0b50', boxSizing: 'border-box' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
                        <h4 style={{ margin: 0, fontSize: '0.9rem', color: isLight ? '#d97706' : '#f59e0b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>📊 Klub Portfólió Válogatás (Vezetői Nézet)</h4>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                          ...
                          <button 
                            ...
                            style={{ 
                              background: isDownloadingThis ? 'var(--border-main, #475569)' : 'linear-gradient(135deg, #f59e0b, #d97706)', 
                              color: isDownloadingThis ? 'var(--text-body, #cbd5e1)' : '#0f172a', 
                              border: 'none', 
                              padding: '6px 12px', 
                              borderRadius: '6px', 
                              fontWeight: 'bold', 
                      ...
                      {sortedUploaders.length === 0 ? (
                        <div style={{ fontSize: '0.85rem', color: 'var(--text-body, #94a3b8)' }}>Még nem érkezett feltöltés a klubtagoktól.</div>
                      ) : (
                        <div style={{ overflowX: 'auto' }}>
                          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                            <thead>
                              <tr style={{ borderBottom: '2px solid var(--border-main, #334155)', textAlign: 'left', color: 'var(--text-body, #94a3b8)' }}>
                            ...
                            <tbody>
                              {sortedUploaders.map(name => {
                                ...
                                return (
                                  <tr key={name} style={{ borderBottom: '1px solid var(--border-main, #1e293b)' }}>
                                    <td style={{ padding: '8px 6px', color: 'var(--text-title, #f8fafc)', fontWeight: 'bold', wordBreak: 'break-word' }}>{name}</td>
                                    <td style={{ padding: '8px 6px', textAlign: 'center', color: userEntries.length >= maxImages ? '#10b981' : 'var(--text-title, #cbd5e1)' }}>{userEntries.length}/{maxImages}</td>
                                    <td style={{ padding: '8px 6px' }}>
                                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                        {userEntries.map((entry, i) => (
                                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: entry.isSelected ? '#10b98120' : 'var(--bg-card, #1e293b)', border: entry.isSelected ? '1px solid #10b98150' : '1px solid var(--border-main, #334155)', padding: '3px 6px', borderRadius: '4px', fontSize: '0.75rem' }}>
                                            <span style={{ color: 'var(--text-title, #e2e8f0)', maxWidth: '100px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{entry.title}</span>
                  ...
                  {!isPast && activeUploadHw !== hw.id && myEntries.length < maxImages && (
                    <button onClick={() => { setActiveUploadHw(hw.id); setHwUploadTitle(''); setHwUploadPreview(null); }} style={{ background: isLight ? '#0284c7' : '#38bdf8', color: isLight ? '#ffffff' : '#0f172a', border: 'none', padding: '10px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', marginBottom: '12px', width: '100%', fontSize: '0.88rem' }}>+ Kép Feltöltése ({myEntries.length}/{maxImages})</button>
                  )}
                  ...
                  {activeUploadHw === hw.id && (
                    <div style={{ background: 'var(--bg-main, #0f172a)', padding: '16px', borderRadius: '12px', marginBottom: '16px', border: '1px solid #38bdf840', boxSizing: 'border-box' }}>
                      <h4 style={{marginTop: 0, color: isLight ? '#0284c7' : '#38bdf8', fontSize: '1rem', marginBottom: '10px'}}>Házi feladat feltöltése</h4>
                      ...
                      <input type="file" accept="image/jpeg, image/png, image/webp" onChange={handleHwFileSelect} style={{ color: 'var(--text-body, #94a3b8)', marginBottom: '12px', width: '100%', fontSize: '0.8rem' }} disabled={isHwUploading} />
                      
                      {hwUploadPreview && <div style={{marginTop: '8px', marginBottom: '16px', textAlign: 'center'}}><img src={hwUploadPreview} alt="Előnézet" referrerPolicy="no-referrer" style={{maxHeight: '220px', borderRadius: '8px', border: '2px solid var(--border-main, #334155)'}} /></div>}
                      
                      <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap'}}>
                        <button onClick={() => handleUploadHw(hw.id)} disabled={isHwUploading} style={{ flex: '1 1 140px', background: isHwUploading ? 'var(--border-main, #475569)' : '#10b981', color: 'white', border: 'none', padding: '10px', borderRadius: '8px', cursor: isHwUploading ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '0.85rem' }}>{isHwUploading ? 'Feltöltés...' : 'Beküldés 🚀'}</button>
                  ...
                  {myEntries.length > 0 && (
                    <div style={{ marginTop: '16px', borderTop: '1px solid var(--border-main, #334155)', paddingTop: '16px' }}>
                      <h4 style={{margin: '0 0 12px 0', fontSize: '1rem', color: 'var(--text-body, #cbd5e1)'}}>Saját beküldött képeid</h4>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '12px' }}>
                        {myEntries.map((entry, index) => {
                          const imageUrl = getImageUrl(entry.drive_file_id, entry.file_url);
                          return (
                            <div key={entry.id} style={{ background: 'var(--bg-main, #0f172a)', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border-main, #334155)' }}>
                              ...
                              {editingHwEntryId === entry.id ? (
                                <div style={{ padding: '8px' }}>
                                  <input value={editHwEntryTitle} onChange={e => setEditHwEntryTitle(e.target.value)} style={{ width: '100%', padding: '6px', marginBottom: '8px', backgroundColor: 'var(--bg-card, #1e293b)', border: '1px solid #38bdf8', color: 'var(--text-title, white)', borderRadius: '4px', boxSizing: 'border-box', fontSize: '0.8rem' }} />
                                ...
                                <div style={{ padding: '8px' }}>
                                  <div style={{ fontSize: '0.82rem', fontWeight: 'bold', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--text-title, #f8fafc)' }}>{entry.title}</div>
                                  {!isPast && (
                                    <div style={{ display: 'flex', gap: '4px', marginTop: '8px', flexWrap: 'wrap' }}>
                                      <button onClick={() => { setEditingHwEntryId(entry.id); setEditHwEntryTitle(entry.title); }} style={{ flex: '1 1 45%', background: 'rgba(56, 189, 248, 0.15)', color: isLight ? '#0284c7' : '#38bdf8', border: 'none', padding: '4px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.78rem' }}>Szerkeszt</button>
                  ...
                  {isPast && (
                    <div style={{ marginTop: '20px', borderTop: isLeader ? '2px dashed #f59e0b' : '1px solid var(--border-main, #334155)', paddingTop: '16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '12px' }}>
                        <div>
                          <h4 style={{margin: '0 0 4px 0', fontSize: '1.05rem', color: isLeader ? (isLight ? '#d97706' : '#f59e0b') : (isLight ? '#0284c7' : '#38bdf8')}}>{isLeader ? '👑 Vezetői Galéria: Eredmények' : '📸 Klub Galéria: Eredmények'}</h4>
                          <p style={{ color: 'var(--text-body, #94a3b8)', fontSize: '0.8rem', margin: 0, wordBreak: 'break-word' }}>Kattints a képre a galéria nézethez. Két nézet között is válthatsz!</p>
                        </div>
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          <button onClick={() => { if (isFilterActive) setFilterSelectedHwIds(prev => prev.filter(id => id !== hw.id)); else setFilterSelectedHwIds(prev => [...prev, hw.id]); }} style={{ background: isFilterActive ? '#10b981' : 'var(--bg-card, #1e293b)', color: isFilterActive ? '#ffffff' : 'var(--text-title, #cbd5e1)', border: isFilterActive ? 'none' : '1px solid var(--border-main, #475569)', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.8rem' }}>{isFilterActive ? '✅ Csak kiválasztottak' : '🔍 Csak kiválasztottak'}</button>
                          <button onClick={() => { if (isSortedByLikes) setSortedHwIds(prev => prev.filter(id => id !== hw.id)); else setSortedHwIds(prev => [...prev, hw.id]); }} style={{ background: isSortedByLikes ? '#ef4444' : 'var(--bg-card, #1e293b)', color: isSortedByLikes ? '#ffffff' : 'var(--text-title, #cbd5e1)', border: isSortedByLikes ? 'none' : '1px solid var(--border-main, #475569)', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.8rem' }}>{isSortedByLikes ? '❤️ Lájk szerint' : '👤 Névsor szerint'}</button>
                        </div>
                      </div>
                      
                      {displayEntries.length === 0 ? (
                        <p style={{ color: 'var(--text-body, #94a3b8)', fontSize: '0.85rem' }}>{isFilterActive ? 'Nincs megjeleníthető kiválasztott kép.' : 'Még senki nem töltött fel képet ehhez a feladathoz.'}</p>
                      ) : (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px' }}>
                          {displayEntries.map((entry, index) => {
                            ...
                            return (
                              <div key={entry.id} style={{ position: 'relative', background: 'var(--bg-main, #0f172a)', borderRadius: '8px', overflow: 'hidden', border: isSelected ? '2px solid #10b981' : (isLeader ? '1px solid #f59e0b50' : '1px solid var(--border-main, #334155)'), display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ padding: '8px', flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '6px' }}>
                                    <div style={{ fontSize: '0.72rem', color: 'var(--text-body, #94a3b8)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}>{entry.user_name}</div>
                                    <button onClick={(e) => { e.stopPropagation(); handleToggleLike(entry.id); }} style={{ background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '3px', padding: '2px 6px', borderRadius: '50px', backgroundColor: entry.user_liked ? '#ef444420' : 'transparent' }}>
                                      <span style={{ fontSize: '0.85rem', color: entry.user_liked ? '#ef4444' : 'var(--text-body, #cbd5e1)' }}>{entry.user_liked ? '❤️' : '🤍'}</span>
                                      <span style={{ color: entry.user_liked ? '#ef4444' : 'var(--text-body, #94a3b8)', fontSize: '0.78rem', fontWeight: 'bold' }}>{entry.like_count || 0}</span>
                                    </button>
                                  </div>
                                </div>
                                ...
                                {isLeader && (
                                  <div style={{ display: 'flex', borderTop: '1px solid #f59e0b40' }}>
                                    <button onClick={() => handleToggleSelect(entry.id)} style={{ flex: 1, background: isSelected ? '#10b98120' : 'transparent', color: isSelected ? '#10b981' : 'var(--text-body, #94a3b8)', border: 'none', padding: '6px', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer' }}>{isSelected ? '❌ Kivesz' : '✅ Kiválaszt'}</button>
                                    <div style={{ width: '1px', background: '#f59e0b40' }}></div>
                                    <a href={entry.drive_file_id ? `https://docs.google.com/uc?export=download&id=${entry.drive_file_id}` : entry.file_url} target="_blank" rel="noreferrer" title="Eredeti felbontású kép letöltése" style={{ flex: 1, textAlign: 'center', background: '#f59e0b15', color: isLight ? '#b45309' : '#f59e0b', padding: '6px', fontSize: '0.75rem', textDecoration: 'none', fontWeight: 'bold' }}>⬇️ Letöltés</a>
                                  </div>
                                )}
                              </div>
                            );
